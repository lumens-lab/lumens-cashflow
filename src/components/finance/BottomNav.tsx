  ...
  <div className="absolute bottom-0 left-0 right-0 z-50 px-5 pb-5 pt-2">
    <div className="glass-strong rounded-[28px] flex items-end justify-between px-3 py-2.5">
      {items.map(({ id, label, Icon }) => {
        ...
        return (
          <button key={id} onClick={() => onChange(id)} className="flex flex-col items-center gap-1 px-3 py-1.5 group" aria-label={label}>
            <div className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all ${isActive ? "bg-primary/20" : ""}`}>
              <Icon className={`w-[18px] h-[18px] transition-colors ${isActive ? "text-primary-glow" : "text-muted-foreground"}`} strokeWidth={isActive ? 2.5 : 2} />
            </div>
            <span className={`font-syne text-[9px] font-bold uppercase tracking-wider transition-colors ${isActive ? "text-primary-glow" : "text-muted-foreground"}`}>
              {label}
            </span>
          </button>
        );
