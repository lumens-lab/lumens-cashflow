            ...
            <div className="relative">
              <div className="flex items-start justify-between">
                ...
                <button onClick={swapDisplayCurrency} disabled={ratesLoading || mainCurrency === subCurrency} className="glass rounded-2xl px-3 py-2 flex items-center gap-1.5 active:scale-95 transition-transform disabled:opacity-50">
                  ...
                  <span className="font-syne text-[10px] font-bold uppercase tracking-wider text-foreground">{displayCurrency}</span>
                </button>
              </div>
              ...
              <div className="flex items-center gap-2 mt-2">
                ...
                <span className="text-[11px] text-muted-foreground">Wallet ID · {profile?.phone || "Add phone"}</span>
              </div>

              <div className="grid grid-cols-2 gap-3 mt-5">
                <div className="glass-subtle rounded-2xl p-3.5">
                  <div className="flex items-center gap-1.5 mb-1.5">
                    <div className="w-6 h-6 rounded-lg bg-success/20 flex items-center justify-center"><ArrowDown className="w-3 h-3 text-success" strokeWidth={3} /></div>
                ...
                <div className="glass-subtle rounded-2xl p-3.5">
                  <div className="flex items-center gap-1.5 mb-1.5">
                    <div className="w-6 h-6 rounded-lg bg-destructive/20 flex items-center justify-center"><ArrowUp className="w-3 h-3 text-destructive" strokeWidth={3} /></div>
        ...
        <div className="px-5 mt-6">
          ...
          <div className="space-y-2">
            ...
            {recent.map((tx) => {
              ...
              return (
                <button key={tx.id} onClick={() => setActionFor(tx)} className="w-full glass rounded-2xl p-3.5 flex items-center gap-3 active:scale-[0.99] transition-transform text-left">
                  <div className={`w-11 h-11 rounded-xl ${bg} flex items-center justify-center`}><Icon className={`w-5 h-5 ${color}`} strokeWidth={2.2} /></div>
                  <div className="flex-1 min-w-0">
                  ...
                  <div className="text-right">
                    <p className={`font-mono-jb text-[14px] font-semibold ${tx.type === "in" ? "text-success" : "text-foreground"}`}>{tx.type === "in" ? "+" : "−"}{format(dispAmt, displayCurrency)}</p>
...
const ActionSheet = ({ tx, onClose, onEdit }: { tx: Transaction; onClose: () => void; onEdit: () => void }) => {
  ...
  return (
    <div className="absolute inset-0 z-[60] flex items-end animate-fade-up">
      ...
      <div className="relative w-full glass-strong rounded-t-[32px] p-5 pb-10">
        ...
        <p className="font-syne text-[10px] font-bold uppercase tracking-wider text-muted-foreground text-center">{tx.category}</p>
        <h3 className="font-syne text-[18px] font-bold text-foreground text-center mt-1">{tx.name}</h3>
        <p className={`font-mono-jb text-[22px] font-semibold text-center mt-1 ${tx.type === "in" ? "text-success" : "text-foreground"}`}>{tx.type === "in" ? "+" : "−"}{format(dispAmt, displayCurrency)}</p>
        <div className="grid grid-cols-2 gap-3 mt-5">
          <button onClick={onEdit} className="glass rounded-2xl py-3 font-syne font-bold text-[11px] uppercase tracking-wider text-foreground">Edit</button>
